package online.jlfsdtc.blog.utils.backup;


import online.jlfsdtc.blog.utils.backup.db.DataTable;
import online.jlfsdtc.blog.utils.backup.db.Row;
    ...
    public String execute() throws SQLException {
        ...
        for (Table table : tables) {
            ...
            if (dataTable.size() > 0) {
                ...
                StringBuilder strBuilder = new StringBuilder("insert into " + quote + table.getName() + quote
                        + " (" + quote + table.getColumns().get(0).getName()
                        + quote);
                for (int i = 1; i < table.getColumns().size(); i++) {
                    strBuilder.append(", ").append(quote).append(table.getColumns().get(i).getName()).append(quote);
                }
                ...
                for (int k = 0; k < dataTable.size(); k++) {
                    ...
                    StringBuilder strBuilder1 = new StringBuilder("(" + getSQLValue(table, row, 0));
                    for (int i = 1; i < dataTable.getColumns().size(); i++) {
                        strBuilder1.append(", ").append(getSQLValue(table, row, i));
                    }
