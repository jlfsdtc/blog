...
import online.jlfsdtc.blog.model.vo.OptionVo;
import online.jlfsdtc.blog.model.vo.UserVo;
import online.jlfsdtc.blog.service.IOptionService;
import online.jlfsdtc.blog.service.IUserService;
import online.jlfsdtc.blog.utils.*;
...
public class BaseInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        //请求拦截处理
        UserVo user = BlogsUtils.getLoginUser(request);
        if (null == user) {
            Integer uid = BlogsUtils.getCookieUid(request);
            if (null != uid) {
        ...
        if (request.getMethod().equals("GET")) {
            String csrf_token = BaseUUID.uu64();
            // 默认存储30分钟
